function largestProductinaSeries(n) {
  ...
  let listedThousandDigits = thousandDigits.map(digit => String(digit)).join('').split('0')

  let maxValue = 0
  listedThousandDigits.forEach((partition) => {
    let list = partition.split('')
    let length = list.length

    if (length > n) {
      for (let i=0; i<= length-n;i++) {
        var slicedList = list.slice(i,i+n)
        var localMax = 1
        slicedList.forEach((n) => {
          localMax*= n
        })
